...
public class Missile : MonoBehaviour
{
    ...
    public void Initialize(MissileData missileData, Vector3 initialVelocity, float intialThrottle = 0)
    {
        this.missileData = missileData;
        Rigidbody.velocity = initialVelocity;
        Rigidbody.angularVelocity = Vector3.zero;
        Rigidbody.ResetInertiaTensor();
        throttleInput = intialThrottle;
    ...
    /// <summary>
    /// Sets a target for this Controller to follow if active is true
    /// </summary>
    /// <param name="target">The exact point in 3d space to fly towards</param>
    /// <param name="active">If target should be ignored/dropped or loop back if missed</param>
    public void SetFlyTarget(Vector3 target, bool active)
    {
        hasTarget = active;
        this.target = target;
    }
    ...
    void UpdateThrust()
    {
        Rigidbody.AddRelativeForce(Throttle * missileData.maxThrust * Vector3.forward);
        if (LocalVelocity.z > missileData.maxSpeed)
        {
            Rigidbody.velocity = Velocity.normalized * missileData.maxSpeed;
        }
    }
    ...
    private void UpdateSteering(float dt)
    {
        ...
        float steeringPower = missileData.steeringCurve.Evaluate(speed); //I'd like to remove the curve but relating turn rate to speed is nice 
        Vector3 targetAV = Vector3.Scale(controlInput, missileData.turnSpeed * steeringPower); //AV is angular velicity........
        Vector3 av = LocalAngularVelocity * Mathf.Rad2Deg;

        Vector3 correction = new(
            CalculateSteering(dt, av.x, targetAV.x, missileData.turnAcceleration.x * steeringPower),
            CalculateSteering(dt, av.y, targetAV.y, missileData.turnAcceleration.y * steeringPower),
            CalculateSteering(dt, av.z, targetAV.z, missileData.turnAcceleration.z * steeringPower)
        );
    ...
    {
        Vector3 localFlyTarget = transform.InverseTransformPoint(flyTarget).normalized * missileData.strength;
        ...
        float wingsLevelInfluence = Mathf.InverseLerp(0f, missileData.aggressiveTurnAngle, angleOffTarget);
